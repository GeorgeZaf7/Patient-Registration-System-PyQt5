...
import sys
import sqlite3
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QMessageBox, QLabel, QFrame, QDesktopWidget, \
    QGridLayout, QLineEdit
from PyQt5.QtGui import QIcon, QFont, QPalette, QLinearGradient, QColor, QBrush
from PyQt5 import QtCore, QtGui
from PyQt5.QtCore import Qt
import Add_New_User
import Search_Register_Patient
...
class Main(QWidget):
    ...
    def btn_submit_clicked(self):
        ...
        if not exists:
            ...
            srname = c.fetchone()
            print(srname[1])
            self.dialogs = Search_Register_Patient.Search(srname[1])
    ...
    def btn_addusr_clicked(self):
        ...
        if not exists:
            ...
            self.txt_User.setText("")
            self.txt_Pass.setText("")
            self.dialogs = Add_New_User.AddUsr()
            #self.dialogs.append(self.dialogs)
            self.dialogs.show()
        conn.commit()
        # Close connection
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QIcon('./icons/Resize_Logo.png'))
    ex = Main()
    ex.show()
    sys.exit(app.exec())
